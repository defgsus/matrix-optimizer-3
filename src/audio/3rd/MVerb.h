...
#ifndef MOSRC_AUDIO_3RD_EMVERB_H
#define MOSRC_AUDIO_3RD_EMVERB_H
...
public:
    ...
    T sampleRate() const { return SampleRate; }

    void process(const T *input0, const T *input1, T *output0, T *output1, int sampleFrames){
        ...
        for(int i=0;i<sampleFrames;++i){
            T oleft = input0 ? input0[i] : 0, left = oleft;
            T oright = input1 ? input1[i] : 0, right = oright;
            MixSmooth += MixDelta;
            ...
            if (output0) output0[i] = left;
            if (output1) output1[i] = right;
        }
    }

    void processMono(const T *input, T *output, int sampleFrames)
    {
        T OneOverSampleFrames = 1. / sampleFrames;
        T MixDelta	= (Mix - MixSmooth) * OneOverSampleFrames;
        T EarlyLateDelta = (EarlyMix - EarlyLateSmooth) * OneOverSampleFrames;
        ...
        for(int i=0;i<sampleFrames;++i){
            ...
            allpassFourTap[2].SetFeedback(Density1);
            T bandwidthLeft = bandwidthFilter[0](left) ;
            T earlyReflectionsL = earlyReflectionsDelayLine[0] ( bandwidthLeft * 0.5 )
                                + earlyReflectionsDelayLine[0].GetIndex(2) * 0.6
                                + earlyReflectionsDelayLine[0].GetIndex(3) * 0.4
                                + earlyReflectionsDelayLine[0].GetIndex(4) * 0.3
                                + earlyReflectionsDelayLine[0].GetIndex(5) * 0.3
                                + earlyReflectionsDelayLine[0].GetIndex(6) * 0.1
                                + earlyReflectionsDelayLine[0].GetIndex(7) * 0.1
                                + ( bandwidthLeft * 0.4 ) * 0.5 ;
            T predelayMonoInput = predelay(( bandwidthLeft ) * 0.5f);
            T smearedInput = predelayMonoInput;
            for(int j=0;j<4;j++)
                smearedInput = allpass[j] ( smearedInput );
            T leftTank = allpassFourTap[0] ( smearedInput + PreviousRightTank ) ;
    ...
    void reset()
    {
        ...
        BandwidthSmooth = BandwidthFreq;
        DampingSmooth = DampingFreq;
        PredelaySmooth = 0;
        SizeSmooth = Size;
        DecaySmooth = Decay;
        DensitySmooth = Density1;
