...
void ObjectGraphScene::createEditMenu()
{
    ...
    // edit object
    if (items.size() == 1)
        ObjectActions::createEditActions(actions, obj, this);

    if (items.size() < 2)
    {
        // new object
        ObjectActions::createNewObjectActions(actions, obj, this, [=](Object*newo)
        {
            QPoint objPos(0,0);
            if (auto item = itemForObject(obj))
                objPos = item->globalGridPos();
            newo->setAttachedData(p_->popupGridPos - objPos, Object::DT_GRAPH_POS);
            p_->nextSelectedObject = newo;
        });
    }
...
void ObjectGraphScene::popup(const QPoint& gridPos)
{
    ...
    // edit object
    if (items.size() == 1)
        ObjectActions::createEditActions(p_->actions, obj, this);

    if (items.size() < 2)
    {
        // new object
        ObjectActions::createNewObjectActions(p_->actions, obj, this, [=](Object*newo)
        {
            QPoint objPos(0,0);
            if (auto item = itemForObject(obj))
                objPos = item->globalGridPos();
...
void ObjectGraphScene::popup(Modulator * mod)
{
    ...
    connect(a, &QAction::triggered, [this, mod]()
    {
        p_->editor->removeModulator(
                    mod->parameter(), mod->modulatorId(), mod->outputId());
    });
...
void ObjectGraphScene::popupObjectDrag(
        Object * source, Object * goal, const QPointF& dropPointF)
{
    ...
    {
        QAction * a = p_->actions.addAction(
                    tr("move into %1").arg(goal->name()), this);
        ...
        ActionList& actions, Object * /*parent*/, const QList<AbstractObjectItem*>& items)
{
    ...
    if (!plural)
    {
        ...
        const bool isClipboard =
                data->formats().contains(ObjectTreeMimeData::objectMimeType);
